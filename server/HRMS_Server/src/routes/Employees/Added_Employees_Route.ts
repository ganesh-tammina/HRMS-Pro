...
getRouter.get("/", async (req: Request, res: Response) => {
    const [rows]: any = await pool.query(
     ...
     FROM employees c
     LEFT JOIN personal_details p ON c.employee_id = p.employee_id
     LEFT JOIN job_details j ON c.employee_id = j.employee_id
     LEFT JOIN offer_details o ON c.employee_id = o.employee_id
     LEFT JOIN employee_credentials e ON c.employee_id = e.employee_id
     LEFT JOIN packagedetails m ON c.employee_id = m.employee_id`
    );
